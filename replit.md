...
- June 20, 2025: Fixed scheduled posts article selection dropdown - resolved issue where articles list was not loading properly in "Tạo bài đăng mới" dialog. Corrected data parsing logic and SelectItem value handling to display all available articles for scheduling.
- June 20, 2025: Enhanced Facebook publishing with native image upload capability - implemented direct image upload to Facebook using Graph API /photos endpoint instead of URL sharing. System now fetches images from URLs, converts to Blob format, and uploads via FormData to Facebook before creating posts. Includes fallback to link posts if image upload fails. Both scheduled posts and publish-now endpoints support this enhanced image handling from featuredImage and platform-specific imageUrls fields.
