...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/social-connections/:id/test', async (req, res) => {
    try {
      ...
      const connection = await storage.getSocialConnection(connectionId);
      if (!connection || connection.userId !== userId) {
        return res.status(404).json({ success: false, error: 'Kết nối không tìm thấy' });
      }

      if (connection.platform !== 'wordpress') {
        return res.status(400).json({ success: false, error: 'Chỉ hỗ trợ test WordPress' });
      }

      const settings = connection.settings as any;
      const { websiteUrl, username, appPassword } = settings;
      
      if (!websiteUrl || !username) {
        return res.status(400).json({ 
          success: false, 
          error: 'Thiếu thông tin kết nối WordPress' 
        });
      }
      ...
      const authPassword = cleanAppPassword || 'test_password';
      ...
      if (response.ok) {
        ...
        res.json({ 
          success: true, 
          message: `Kết nối thành công với ${userData.name || username}`,
          data: { username: userData.username, name: userData.name }
        });
      } else {
        const errorData = await response.text();
        res.json({ 
          success: false, 
          error: `Lỗi kết nối WordPress: ${response.status} - ${errorData}`,
          details: {
            status: response.status,
            statusText: response.statusText,
            url: wpApiUrl,
      ...
      res.status(500).json({ 
        success: false, 
        error: 'Lỗi khi test kết nối: ' + error.message 
      });
    }
  });
